...
tm.copytask = function(from, to, callback) {
    ...
    fs.stat(turi.pathname, function(err, stat) {
        ...
        function loop(args) {
            if (!args.s) return require('./source')(from, function(err, s) {
                if (err) return callback(err);
                args.s = s;
            ...
            if (!args.info) return args.s.getInfo(function(err, info) {
                if (err) return callback(err);
                args.info = info;
                loop(args);
            });
            if (!args.t) return tilelive.load(tmp, function(err, t) {
                if (err) return callback(err);
                args.t = t;
                loop(args);
            });
            if (!args.task) return (function() {
                var opts = {
                    minzoom: args.info.minzoom,
                    maxzoom: args.info.maxzoom,
                    bbox: args.s.data.bounds || require('./source').extent(args.s.data)
                };
                var scheme = tilelive.Scheme.create(args.s.data._prefs.scheme || 'scanline', opts);
                args.task = new tilelive.CopyTask(from, tmp, scheme);
                tm._copy = {
                    id: from,
                    file: null,
                    stat: null,
                    task: args.task,
                    stats: args.task.stats.snapshot()
                };
                console.log('[%s] %s (z%d-%d @ %s)',
                    ...
                    opts.minzoom,
                    opts.maxzoom,
                    opts.bbox.map(function(v) { return v.toFixed(4) }).join(','));
                ...
                // @TODO process.nextTick() does not work here to ensure the
                // event is fired after teh callback has a chance to run. Why?
                setTimeout(function() { tm._copy.task.emit('started'); },100);
                loop(args);
            })();
            if (!args.geocoder) return (function() {
                // No geocoder_layer in source info. Skip.
                if (!args.info.geocoder_layer) {
                    args.geocoder = true;
                    return loop(args);
                }
                var c = new Carmen({from:args.s, to:args.t});
                var total = 0;
                var shardlevel = (function(z) {
                    if (z > 12) return 3;
                    if (z > 8)  return 2;
                    if (z > 6)  return 1;
                    return 0;
                ...
                args.geocoder = c;
                c.on('open', function() {
                    args.t._geocoder.shardlevel = shardlevel;
                    c.on('index', function(count) {
                        total += count;
                        console.log('[%s] %s indexed %s docs ...',
                            (new Date).toUTCString(),
                            path.basename(from),
                            total);
                    });
                    console.log('[%s] %s geocoder (shardlevel=%s, layer=%s, res=%s)',
                        (new Date).toUTCString(),
                        path.basename(from),
                        shardlevel,
                        args.info.geocoder_layer,
                        args.info.geocoder_resolution || 0);
                    c.index(args.s, args.t, {shardlevel:shardlevel}, function(err) {
                        if (err) return (tm._copy = err) && console.error(err);
                        console.log('[%s] %s geocoder indexing complete',
                            (new Date).toUTCString(),
                            path.basename(from));
                        loop(args);
                    });
            ...
            // Task was canceled.
            if (args.task.abort) return;

            // Start the task.
            args.task.start(function(err) {
                if (err) return (tm._copy = err) && console.error(err);
                args.task.sink.putInfo(args.info, function(err) {
                    if (err) return (tm._copy = err) && console.error(err);
                });
            });

            // Move tmp export into place when complete.
            args.task.on('finished', function(err) {
               // Enable caching on source again.
                delete args.s._nocache;

                fs.unlink(args.task.job, function(err) {
                    if (err) console.error(err);
                });
...
tm.cleartask = function(callback) {
    if (!tm._copy) return callback();
    if (tm._copy.task && tm._copy.task.source && tm._copy.task.sink) return tm._copy.task.pause(function(err) {
        if (err) return callback(err);
        // Remove sink file.
    ...
    // There may be a task at this point but it is not running or it is in
    // an earlier phase like geocoder indexing which we cannot pause. Let it
    // do its thing and die.
    if (tm._copy.task) tm._copy.task.abort = true;
    tm._copy = null;
    return callback();
};
